...
Toutes les URL qui commencent par HTTP utilisent un protocole de transfert hypertexte, c’est grâce à ce protocole de réseaux standard que les navigateurs et les serveurs peuvent communiquer en échangeant des données.
...
L’unique objectif de HTTP est d’afficher les informations demandées sans se soucier de la façon dont ces informations se déplacent d’un endroit à un autre. Cela signifie que HTTP peut être intercepté et éventuellement détourné, ce qui rend les informations et leurs destinataires vulnérables.
...
En gros, le protocole HTTPS est une extension de HTTP. Le « S » à la fin est l’initiale du mot « Secure » (sécurisé) et il fonctionne grâce au protocole TLS (Transport Layer Security), la technologie de sécurité standard pour établir une connexion chiffrée entre un serveur web et un navigateur.

Sans la présence de HTTPS, toutes les données que vous entrez sur un site (ex : nom d’utilisateur, mot de passe) seront envoyées en format de texte brute et seront, par conséquent, vulnérables aux interceptions et à l’espionnage.
